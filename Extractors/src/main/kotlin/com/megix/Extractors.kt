    ...
    override suspend fun getUrl(
        ...
        document.select("div.text-center a").amap {
            ...
            if(text.contains("DIRECT DL")) {
                ...
                try {
                    val link = it.attr("href")
                    app.get("https://new6.gdflix.dad$link")
                    .document.select("a.btn.btn-outline-info").amap {
                        val serverUrl = "https://new6.gdflix.dad" + it.attr("href")
            ...
            else if (text.contains("Instant DL"))
            {
                try {
                    ...
                    val link = app.get(instantLink, allowRedirects = false).headers["location"]?.substringAfter("url=") ?: ""
                    callback.invoke(
                        newExtractorLink(
