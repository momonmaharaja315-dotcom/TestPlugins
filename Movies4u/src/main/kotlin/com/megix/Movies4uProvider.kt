...
class Movies4uProvider : MainAPI() { // all providers must be an instance of MainAPI
    ...
    override suspend fun loadLinks(
        ...
        sources.mapNotNull {
            ...
            doc2.select("a:has(button:matches((?i)(Download)))").mapNotNull { aTag->
                val link = if(aTag.attr("href").contains("https")) aTag.attr("href") else "https://link.ilink.lol" + aTag.attr("href")
                callback.invoke(
                    ExtractorLink(
                        "Movies4u",
                        "Movies4u $size",
